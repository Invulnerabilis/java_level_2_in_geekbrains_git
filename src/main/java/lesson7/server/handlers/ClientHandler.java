        ...
        while (true) {
            ...
            switch (typeMessage) {
                ...
                case PRIVATE_MSG_CMD_PREFIX -> {
                    if (myServer.isUsernameBusy(data[1])) {
                        List<ClientHandler> clientHandlerList = myServer.getClients();
                        for (ClientHandler client :
                                clientHandlerList) {
                            if (client.username.equals(data[1])) {
                                client.out.writeUTF(username + ": " + stringBuilder);
                            }
                        }
                    } else {
                        sendMessage("", data[1] + " isn't auth!");
                    }
                }
